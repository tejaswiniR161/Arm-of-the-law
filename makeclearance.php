...
$flaf=0;
            ...
            $conn = new mysqli($servername, $username, $password, $dbname);
                ...
                if($flaf==1)
                {
                    ...
                    $sql="UPDATE `cases` SET `c_cstatus` = 'Ongoing', `c_cstatus_i` = '3', `c_cleared` = '1', `c_cldate` = '$d', `c_cltime` = '$t', `c_cl_pl_id` = '$plid', `c_ongoing` = '1', `c_gdate` = '$d', `c_gtime` = '$t' WHERE `cases`.`c_id` = $cid ";

                    if ($conn->query($sql) === TRUE)
                    {
                    echo "<script> alert('Case Clearance Successful!!'); </script>";
            ...
            $body = "Greetings,\n\n Your Case Is Cleared. check under cases menu in AOTL home page to know more!  \n Case ID : ";
            $body .= $cid ."\n\n Good Luck !! \n -Team AOTL";
            ...
            header("Refresh:0; url=updatecasedetails.php");
                ...
                elseif($flaf==2)
                {
                    echo "<script> alert('Case Has Already Been Cleared!!'); </script>";
                    header("Refresh:0; url=updatecasedetails.php");
                ...
                elseif($flaf==0)
                {
                    echo "<script> alert('Case Does Not Exist Or You Do Not Have Permission To Clear The Case!!'); </script>";
                    header("Refresh:0; url=updatecasedetails.php");
